"""TCAM-Attn1D — Time/Channel Attention 1D-CNN (literature baseline).

Paper name / registry key
-------------------------
* **Paper name:** TCAM-Attn1D (reimplementation of Xu et al., 2024)
* **Class:** ``TCAMAttn1DNet``
* **Config ``model_name``:** ``tcam_attn1d``
* **Legacy aliases:** ``TCAM1DCNN``, ``tcam1dcnn``

Layer character
---------------
Six Conv1d modules, each followed by a **Time-Channel Attention Module (TCAM)**
(TAM then CAM). Designed for short frames (e.g. 8000 samples = 0.5 s @ 16 kHz)
with multi-frame SUM inference — **not** the deployable full-clip student.
